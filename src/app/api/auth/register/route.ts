...
const registerDataStandards = z.object({
    name: z.string({ error: (issue) => (issue.input === undefined ? 'NAME_REQUIRED' : undefined) }).trim().min(2, 'NAME_TOO_SHORT').max(50, 'NAME_TOO_LONG').regex(/^[\u0600-\u06FFa-zA-Z\s]+$/, 'NAME_INVALID_CHARACTERS'),
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const newUser = await prisma.user.create({
            data: {
                name, email, passwordHash, phone, role: 'customer'
            },
            select: {
